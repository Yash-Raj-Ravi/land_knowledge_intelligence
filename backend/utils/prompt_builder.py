def build_land_rag_prompt(context: str, query: str) -> str:
...
4. CITATION REQUIREMENT: For every document claim in your answer, you MUST provide matching citations under the "citations" field. Each citation must use the exact `document_id`, `file_name`, and `page_number` specified in [DOCUMENT EVIDENCE]. Do NOT invent document IDs or page numbers.
5. EVIDENCE COVERAGE:
...
6. INSUFFICIENT INFORMATION: If context is insufficient, state clearly what is known and what is missing, and set "evidence_coverage" to "INSUFFICIENT".
7. NUMERICAL ACCURACY: State exact numbers, units (Hectares, Acres, Sq meters), survey numbers, and currency values precisely as given.
8. NO ASSUMPTIONS: Do not assume parcel status or ownership if it is not explicitly documented.
9. PROFESSIONAL TONE: Provide clean, objective, structured natural language explanations.
10. DOCUMENT COUNTING: When asked how many documents exist or are available, count distinct documents by unique `document_id` or `file_name`. Multiple pages or chunks from the same document ID belong to 1 single document. Do NOT count multiple pages of the same document as separate documents.
11. OUTPUT FORMAT: Output ONLY valid JSON matching the exact schema below.
